...
- [x] Drop `TON` from the asset allowlist — it is not listed on Binance USDT-M futures. *(Found while verifying example commands for the demo: `Show me the structure of TON` failed with `MARKET_DATA_ERROR`. Binance answers HTTP 200 with an empty `{}` body for an unlisted symbol, so the schema rejected the shape and it read like a malformed response rather than a missing market. TON was offered twice — as a picker chip and as a symbol commands could name — so it was a dead button, not an edge case. Checked all ten against the exchange: nine return a real ticker, TON does not. Removed it, deleted its logo, and added `tests/market/supported-assets.test.ts`, which asks Binance directly and skips loudly when the exchange is unreachable rather than failing on an offline laptop.)*
...
- [x] Rewrite `DEMO.md` as the submission video script, and re-shoot the README captures. *(The old script was pure walkthrough with no problem/what-we-built/why-this-track framing, which is what a submission video is judged on; it is now seven beats for 3:00 with `[CUT FIRST]` markers down to 2:00, opening with an `/api/health` check so the person recording knows which provider the meta bar will name before rolling rather than discovering it on camera. Binance own spec for the video is not recorded anywhere in this repo, so that assumption is stated at the top of the file rather than left implicit. The README captures were re-shot now that the cloud provider actually answers — the hero meta bar reads `GEMINI · GEMINI-3.6-FLASH` instead of the fallback.)*
