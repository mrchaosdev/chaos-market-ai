...
export function WorkflowMetaBar({ meta, runId, workflow }: WorkflowMetaBarProps) {
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border border-border bg-surface px-4 py-3 font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
      ...
      <span className="ml-auto text-subtle-foreground">{runId}</span>
    </div>
  );
}
...
function Item({ icon, label, tone = "default" }: { icon: React.ReactNode; label: string; tone?: "default" | "warning" }) {
  return (
    <span className={`flex items-center gap-2 ${tone === "warning" ? "text-warning" : ""}`}>
      {icon}
      {label}
    </span>
  );
